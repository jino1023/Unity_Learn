...
public class Test : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("UnlockAchievement_first"))
            GPGSBinder.Inst.UnlockAchievement(GPGSIds.achievement_first, success => log = $"{success}");

        if (GUILayout.Button("UnlockAchievement_second"))
            GPGSBinder.Inst.UnlockAchievement(GPGSIds.achievement_second, success => log = $"{success}");
        ...
        if (GUILayout.Button("ShowTargetLeaderboardUI_num"))
            GPGSBinder.Inst.ShowTargetLeaderboardUI(GPGSIds.leaderboard_scored);

        if (GUILayout.Button("ReportLeaderboard_num"))
            GPGSBinder.Inst.ReportLeaderboard(GPGSIds.leaderboard_scored, 1000, success => log = $"{success}");

        if (GUILayout.Button("LoadAllLeaderboardArray_num"))
            GPGSBinder.Inst.LoadAllLeaderboardArray(GPGSIds.leaderboard_scored, scores =>
            {
                log = "";
        ...
        if (GUILayout.Button("LoadCustomLeaderboardArray_num"))
            GPGSBinder.Inst.LoadCustomLeaderboardArray(GPGSIds.leaderboard_scored, 10,
